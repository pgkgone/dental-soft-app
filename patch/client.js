...
Client.prototype._invoke = function(method, args, location, callback) {
  ...
  xml = "<Envelope " +
    ...
    "</Body>" +
    "</Envelope>";
    xml=(xml.replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&').replace('<tns:_xml>','').replace('</tns:_xml>','').replace('/tns:','/')) //.replace('tns:','')
    var k =xml.search('tns:')
    var str=""
    for(var i=4;xml[i+k]!='>';i++){
      str+=xml[i+k]
    }
    xml=xml.replace('tns:'+str,str+' xmlns="urn:grvssl"')
    console.log(xml)
  http.request(
    location,
    xml,
